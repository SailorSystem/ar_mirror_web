  ...
  useEffect(() => {
    ...
    import('https://unpkg.com/three@0.164.1/build/three.module.js')
      .then((threeModule) => {
        ...
        startThreeMirrorScene(threeModule);
      })
      .catch(() => {
        if (cancelled || started) return;
        startFallbackMirrorScene();
      });
    ...
    function animateFallbackScene() {
      ...
      const canvas2 = fallbackContext.canvas;
      const width = canvas2.clientWidth || window.innerWidth;
      const height = canvas2.clientHeight || window.innerHeight;
    ...
    function resizeHomeScene() {
      const canvas2 = renderer ? renderer.domElement : fallbackContext.canvas;
      const width = canvas2.clientWidth || window.innerWidth;
      const height = canvas2.clientHeight || window.innerHeight;
    ...
    return () => {
      ...
      window.removeEventListener('resize', resizeHomeScene);
