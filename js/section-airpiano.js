...
// Frecuencias: C4 a C5 (13 notas cromáticas)
const NOTES = [
  261.63, // C4
  277.18, // C#4
  293.66, // D4
...
// Mapa: tecla blanca i → índice cromático
const WHITE_TO_CHROMATIC = [0, 2, 4, 5, 7, 9, 11, 12];
// Pares (blanca_i, blanca_i+1) que tienen tecla negra entre ellas
const BLACK_PAIRS = [
  { between: [0,1], chromatic: 1  }, // C#4
  { between: [1,2], chromatic: 3  }, // D#4
  { between: [3,4], chromatic: 6  }, // F#4
  { between: [4,5], chromatic: 8  }, // G#4
  { between: [5,6], chromatic: 10 }, // A#4
];
  ...
  if (video.readyState === 4) {
    ...
    if (result.landmarks?.length) {
      result.landmarks.forEach((lm) => {
        ...
        if (y > kbY) {
          // Primero verificar teclas negras (z-order superior)
          let hitKey = -1;
          let isBlack = false;
          ...
          // Highlight de la tecla presionada
          if (isBlack) {
